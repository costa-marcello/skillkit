---
name: gemini
description: Runs Gemini CLI for code review, plan review, or large context (>200k) processing. Use when the user asks to run Gemini CLI or references Google Gemini for comprehensive analysis requiring large context windows. Uses Gemini 3 Pro by default for state-of-the-art reasoning and coding.
context: fork
---
...
## When to Use Gemini
- **Frontend Development**: Building UI components, pages, and applications with exceptional design quality (Gemini 3 Pro excels at frontend implementation)
...
| --- | --- | --- | --- |
| `gemini-3-pro-preview` ⭐ | **Flagship model**: Complex reasoning, coding, frontend UI, agentic tasks | 1M input / 64k output | Exceptional frontend/UI quality, vibe coding, 76.2% SWE-bench, $2-4/M input |
...
### Frontend Development (Background/Automated)
```bash
# Gemini 3 Pro excels at frontend implementation with high design quality
gemini -m gemini-3-pro-preview --approval-mode yolo \
  "Build a responsive dashboard component with:
   1. Clean, modern UI design
   2. Smooth animations and transitions
   3. Accessible markup (ARIA labels, keyboard nav)
