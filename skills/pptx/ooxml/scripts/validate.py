...
def main():
    ...
    if not unpacked_dir.is_dir():
        raise ValueError(f"Unpacked directory does not exist or is not a directory: {unpacked_dir}")
    if not original_file.is_file():
        raise ValueError(f"Original file does not exist or is not a file: {original_file}")
    if file_extension not in [".docx", ".pptx", ".xlsx"]:
        raise ValueError(f"Unsupported file type '{file_extension}'. Must be .docx, .pptx, or .xlsx")
